...
class GroupFragment : ToolbarFragment(R.layout.layout_group),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (BuildConfig.CUSTOM_LOCKED_APP) {
            ...
            toolbar.menu.findItem(R.id.action_new_group)?.isVisible = false
        }
        ...
        ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(
            ...
            override fun getSwipeDirs(
                ...
                if (BuildConfig.CUSTOM_LOCKED_APP) return 0
    ...
    inner class GroupHolder(binding: LayoutGroupItemBinding) :
        ...
        override fun onMenuItemClick(item: MenuItem): Boolean {
            if (BuildConfig.CUSTOM_LOCKED_APP) {
                activity.snackbar(R.string.configuration_locked).show()
                return true
            }
            ...
            fun isProtectedAction() = when (item.itemId) {
                R.id.action_universal_qr,
                R.id.action_universal_clipboard,
                R.id.action_export_clipboard,
                R.id.action_export_file,
                R.id.action_export_managed_unlocked_clipboard,
            ...
            if (isProtectedAction() &&
                ConfigLock.hasLockedProfiles(SagerDatabase.proxyDao.getByGroup(proxyGroup.id))
            ) {
                activity.snackbar(R.string.configuration_locked).show()
                return true
            }
            ...
            when (item.itemId) {
                ...
                R.id.action_export_managed_locked_clipboard -> {
                    runOnDefaultDispatcher {
                        val profiles = SagerDatabase.proxyDao.getByGroup(selectedGroup.id)
                        val link = ManagedConfig.exportProfiles(profiles, true)
                        onMainDispatcher {
                            export(link)
                        }
                    }
                ...
                R.id.action_export_managed_unlocked_clipboard -> {
                    runOnDefaultDispatcher {
                        val profiles = SagerDatabase.proxyDao.getByGroup(selectedGroup.id)
                        val link = ManagedConfig.exportProfiles(profiles, false)
                        onMainDispatcher {
                            export(link)
                        }
                    }
        ...
        fun bind(group: ProxyGroup) {
            ...
            editButton.isGone = BuildConfig.CUSTOM_LOCKED_APP || proxyGroup.ungrouped
            updateButton.isInvisible =
                BuildConfig.CUSTOM_LOCKED_APP || proxyGroup.type != GroupType.SUBSCRIPTION
            optionsButton.isGone = BuildConfig.CUSTOM_LOCKED_APP
            groupName.text = proxyGroup.displayName()

            editButton.setOnClickListener {
                if (BuildConfig.CUSTOM_LOCKED_APP) return@setOnClickListener
