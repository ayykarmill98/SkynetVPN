...
abstract class ProfileSettingsActivity<T : AbstractBean>(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val intentEditingId = intent.getLongExtra(EXTRA_PROFILE_ID, 0L)
        if (ConfigLock.isProfileLocked(intentEditingId)) {
            finish()
            return
        }

        if (savedInstanceState == null) {
            val editingId = intentEditingId
    ...
    open suspend fun saveAndExit() {
        if (ConfigLock.isProfileLocked(DataStore.editingId)) {
            finish()
            return
        }
