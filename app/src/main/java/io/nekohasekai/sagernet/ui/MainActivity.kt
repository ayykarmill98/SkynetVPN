...
class MainActivity : ThemedActivity(),
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        if (ManagedConfig.isManagedConfig(link)) {
            runOnDefaultDispatcher {
                importManagedConfig(link)
            }
            return
        }
    ...
    suspend fun importManagedConfig(link: String) {
        val managedConfig = try {
            ManagedConfig.parse(link)
        } catch (e: Exception) {
            onMainDispatcher {
                alert(e.readableMessage).show()
            }
        ...
        onMainDispatcher {
            ...
            snackbar(
                resources.getQuantityString(
                    R.plurals.added, created.size, created.size
                )
            ).show()
        }
    }
